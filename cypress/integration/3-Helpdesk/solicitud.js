...
const crear = require('../../fixtures/solicitud.json');
...
class solicitud {
    ...
    elementssol() {
                ...
                switch (asun.format) {
                    ...
                    case 'Guia comidas{enter}':
                        ...
                        cy.get("iframe").then(($iframe) => {
                            ...
                            cy.wrap($body).click().type(asun.describe,'{enter}');
                        });
                        break;
        ...
        cy.get('.form-group').eq(9).dblclick({force:true}).type('AGENTE{enter}');
        cy.get('input.p-0').eq(0).check();
        cy.get('.modal-footer > .btn-primary').click({force:true});
        cy.wait(5000);
        cy.get('div.asset-list-container').contains('Departamento: Financiera Ubicación: Sede Occidente ')
